    ...
    async (req, res) => {
        try {
            ...
            return res.status(500).json({
                error: `Internal server error ` + `while creating the transaction.`,
            })
        }
    }
)
...
// get one
transactions.post('/:transaction_id', checkAccountID, checkTransactionID, async (req, res) => {
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (deletedTransaction.error || deletedTransaction.err) {
                return res
                    .status(500)
                    .json({ error: deletedTransaction.error || deletedTransaction.err })
            }
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!updatedTransaction) {
                return res
                    .status(404)
                    .json({ error: 'Transaction not found or no changes were made.' })
            }
