...
// start login
accounts.post('/login-initiate', checkEmailProvided, checkPasswordProvided, async (req, res) => {
    try {
        ...
        await updateAccountMFAOneTimePassword(
            oneAccount.account_id,
            hashedOtp,
            expirationTimeForOTP
        )
...
// complete login
accounts.post('/verify-otp', async (req, res) => {
    ...
    try {
        ...
        const isNewDevice = !previousLogins?.some(
            (login) =>
                login.ip_address === ip_address && login.device_fingerprint === device_fingerprint
        )
        if (isNewDevice) {
            const mailOptionsNewDevice = createMailOptions(
                account.email,
...
// delete account
accounts.delete('/:account_id', verifyToken, checkAccountIndex, async (req, res) => {
    try {
        ...
        try {
            ...
            if (updatedAccount.account_id) {
                ...
                const mailOptions = createMailOptions(
                    updatedAccount.email,
                    'Security Update',
                    `Your password has been recently updated. If this wasn\'t you, ` +
...
// guest login
accounts.post('/guest-login', async (req, res) => {
    try {
        ...
        if (!guestAccount) {
            return res
                .status(500)
                .json({ error: 'Guest account is not available. Please try again later.' })
        }
